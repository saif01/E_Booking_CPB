...
$query=mysqli_query($con,"SELECT cms_hard_complain.hard_id, cms_hard_complain.user_id, cms_hard_complain.tools, cms_hard_complain.details, cms_hard_complain.documents, cms_hard_complain.status, cms_hard_complain.warrenty, cms_hard_complain.delivery, cms_hard_complain.last_up, cms_hard_complain.reg, cms_hard_category.category, cms_hard_subcategory.subcategory, user.user_name, user.user_dept, cms_delivery.deliver_id,cms_delivery.name FROM cms_hard_complain INNER JOIN cms_hard_category ON cms_hard_complain.cat_id=cms_hard_category.cat_id INNER JOIN cms_hard_subcategory ON cms_hard_complain.sub_id=cms_hard_subcategory.sub_id INNER JOIN user ON cms_hard_complain.user_id=user.user_id INNER JOIN cms_delivery ON cms_hard_complain.hard_id=cms_delivery.hard_id WHERE cms_hard_complain.delivery='Delivered' ORDER BY cms_hard_complain.hard_id DESC");
...
<a href="javascript:void(0);" onClick="popUpWindow('user-profile?user_id=<?php echo ($row['user_id']); ?>');" title="User Details"><?php echo ($row['user_name']); ?></a> 
                       ...
                       <td>
<a href="javascript:void(0);" onClick="popUpWindow('deliver-profile?deliver_id=<?php echo ($row['deliver_id']); ?>');" title="User Details"><?php echo ($row['name']); ?></a>
                      </td>
                     <td>
...
?>
           ...
           title="Action">  <span class="btn btn-danger"><i class="fa fa-edit text-success"></i>Details</span> </a>
                        </td>
